...
then
	...
	cp $CMSSW_BASE/src/cms_lpc_llp/llp_timing_ntupler/condor_scripts/${config} ./
	#get grid proxy
	...
	echo "************************************"
        ...
        sed -i "s\/mnt/hadoop\'file:/mnt/hadoop\g" inputfilelistForThisJob_${jobnumber}.txt
	...
	##^_^##
	echo "run_signal_aod_job.sh finished"
	date
